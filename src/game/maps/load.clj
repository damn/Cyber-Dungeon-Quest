(nsx game.maps.load
  (:require [game.maps.data :as data]
            [game.creatures.core :refer (try-spawn)]
            game.player.core))
...
(defn load-maps-content
  ...
  []
  ; TODO assert spawns !!
  (try-spawn :vampire ; <+ this main game config should not be here spread out through the code (also skills damage tc.)
